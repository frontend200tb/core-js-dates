...
function getNextFriday(date) {
  ...
  const num = dayWeek < 5 ? 5 - dayWeek : 12 - dayWeek;
  time.setDate(day + num);
  return time;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return 1 + Math.floor(ms / (1000 * 60 * 60 * 24));
}
...
function isDateInPeriod(date, period) {
  const oDate = new Date(date).getTime();
  const oStart = new Date(period.start).getTime();
  const oEnd = new Date(period.end).getTime();

  return oDate >= oStart && oDate <= oEnd;
}
...
function formatDate(date) {
  ...
  const m = oDate.getUTCMonth() + 1;
  const d = oDate.getUTCDate();
  const y = oDate.getUTCFullYear();
  let h = oDate.getUTCHours();
  let mm = oDate.getUTCMinutes();
  let ss = oDate.getUTCSeconds();
  let ampm = 'AM';
  if (h >= 12) {
    ampm = 'PM';
  }
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= daysInMonth; i += 1) {
    const j = new Date(year, month - 1, i).getDay();
    if (j === 6 || j === 0) {
      result += 1;
    }
  }
...
function getWeekNumberByDate(date) {
  ...
  const idate = new Date(year, 0, 1);
  return Math.ceil(((date - idate) / msInDay + idate.getDay() + 1) / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  if (d < 13) {
    const r = new Date(y, m, 13).getDay();
    if (r === 5) {
      return new Date(y, m, 13);
    }
  }
  for (let i = 1; i < 20; i += 1) {
    const next = new Date(y, m + i, 13);
    const r = next.getDay();
    if (r === 5) {
      return new Date(y, m + i, 13);
    }
  }
  return new Date(y, m, 13);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (day <= end) {
    if (workDay) {
      for (let i = 0; i < countWorkDays && day <= end; i += 1) {
        result.push(day.toJSON());
  ...
  return result.map((elem) => elem.slice(0, 10).split('-').reverse().join('-'));
}
